...
class FuncionarioSolicitacaoController extends Controller
{
    /**
    ...
    public function show(Solicitacao $solicitacao)
    {
        ...
        if(Auth::user()->tipo=='funcionario'){
            ...
            if($encaminhamento && Auth::user()->funcionario->acesso=='A'){

                $encaminhamento->update(['lida'=>'1']);
            }
        }
       $funcionarios = Funcionario::where('departamento_id', Auth::user()->funcionario->departamento_id)->get();
        return view('Funcionario.Solicitacoes.show',compact(['solicitacao','departamentos','encaminhamento','funcionarios']));
    }
    public function encaminhamento(Encaminhamento $encaminhamento)
    {
        $solicitacao = Solicitacao::find($encaminhamento->solicitacao_id);;
